...
TEST_F(TestOsTask, os_task_create_finite_with_without_param_ok) // NOLINT
{
    ...
    ASSERT_TRUE(os_task_create_finite_without_param(&task_func_without_param, task_name, stack_depth, priority));
...
TEST_F(TestOsTask, os_task_create_finite_without_param_fail) // NOLINT
{
    ...
    ASSERT_FALSE(os_task_create_finite_without_param(&task_func_without_param, task_name, stack_depth, priority));
    TEST_CHECK_LOG_RECORD(
        ESP_LOG_INFO,
        "[my_task_name2] Start thread 'my_task_name2' with priority 3, stack size 2048 bytes");
