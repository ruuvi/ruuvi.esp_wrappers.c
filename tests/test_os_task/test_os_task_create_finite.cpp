/**
 * @file test_os_task_create_finite.cpp
 * @author TheSomeMan
 * @date 2020-12-08
...
TEST_F(TestOsTask, os_task_create_finite_ok) // NOLINT
{
    ...
    ASSERT_TRUE(os_task_create_finite(&task_func, task_name, stack_depth, &this->m_taskParam, priority));
...
TEST_F(TestOsTask, os_task_create_finite_fail) // NOLINT
{
    const char*              task_name   = "my_task_name2";
    const uint32_t           stack_depth = 2048;
    const os_task_priority_t priority    = 3;
    ...
    ASSERT_FALSE(os_task_create_finite(&task_func, task_name, stack_depth, &this->m_taskParam, priority));
    TEST_CHECK_LOG_RECORD_WITH_THREAD(
        ESP_LOG_INFO,
        "my_task_name2",
        0,
